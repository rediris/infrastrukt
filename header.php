...
<div class="nav-wrapper <?php echo $topbar_classes;?>">
  <nav class="top-bar" data-topbar data-options="is_hover:<?php echo $topbar_hover;?>">
    ...
    <section class="top-bar-section" role="navigation">
      <?php wp_nav_menu( array( 'theme_location' => 'header-menu', 'menu_class' => 'left', 'container' => '', 'container_class' => '', 'fallback_cb' => 'foundation_page_menu', 'walker' => new foundation_navigation() ) ); ?>
    </section>
  </nav>
</div><!--/.nav-wrapper-->
<?php $header =  get_header_textcolor();
if ( $header !== "blank" ) : ?>
<header class="site-header" <?php $header_image = get_header_image(); if ( ! empty( $header_image ) ) : ?> style="background-image:url('<?php echo esc_url( $header_image ); ?>');" <?php endif; ?>>
  <div class="row">
